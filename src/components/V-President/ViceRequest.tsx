...
const ViceRequest = () => {
  const [pendingRequests, setPendingRequests] = useState<ViceDeanRequest[]>([]);
  const [loading, setLoading] = useState(true);
  const [processingId, setProcessingId] = useState<number | null>(null);
  const [selectedRequest, setSelectedRequest] = useState<ViceDeanRequest | null>(null);
  const [showModal, setShowModal] = useState(false);
  const [viceNotes, setViceNotes] = useState("");
  const [actionType, setActionType] = useState<"Approved" | "Rejected">("Approved");
  const [currentUser, setCurrentUser] = useState<any>(null);
  const [requestTypeFilter, setRequestTypeFilter] = useState<"All" | "Gate Pass" | "Leave" | "Loan">("All");

  useEffect(() => {
    checkViceAuthorization();
    fetchPendingRequests();
  }, []);

  const checkViceAuthorization = async () => {
    try {
      ...
      if (userData.role !== "Vice President") {
        toast.error("Access denied. Only Vice President can access this page.");
        return;
      }

      setCurrentUser(userData);
    } catch (error) {
      console.error("[ViceRequest] Authorization error:", error);
  ...
  const fetchPendingRequests = async () => {
    setLoading(true);
    try {
      console.log("[ViceRequest] Fetching dean-related requests for Vice President view...");
      ...
      if (requestError) {
        console.error("[ViceRequest] Error fetching requests:", requestError);
      ...
      const requestsWithUsers: ViceDeanRequest[] = [];
      ...
      if (usersError) {
        console.error("[ViceRequest] Error fetching users for requests:", usersError);
      }
      ...
      try {
        ...
        console.log("[ViceRequest] dean_approvals table not available, using requests table data only");
      }
    } catch (error) {
      console.error("[ViceRequest] Error fetching pending requests:", error);
  ...
  const handleApprovalAction = (request: ViceDeanRequest, action: "Approved" | "Rejected") => {
    ...
    try {
      ...
      if (!approverId) {
        ...
        if (approverError || !approverData) {
          console.error("[ViceRequest] Unable to resolve approver user:", approverError);
      ...
      try {
        await supabase
          ...
          .update({
            ...
            approved_by: approverId,
            approved_date: now,
            dean_notes: viceNotes || null,
            updated_at: now,
          })
          .eq("request_id", selectedRequest.request_id);
      } catch (err) {
        console.log("[ViceRequest] dean_approvals update skipped or failed:", err);
      }
      ...
      console.error("[ViceRequest] Error processing approval:", error);
      ...
      <main className="flex flex-col w-full max-w-6xl mx-auto p-4 sm:p-6 bg-white border border-gray-200 shadow-xl rounded-2xl space-y-6">
        <section className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl sm:text-2xl font-semibold text-gray-900 tracking-tight">Vice President Dean Requests</h1>
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
            <div className="bg-white rounded-2xl p-8 w-full max-w-lg mx-4 shadow-2xl border border-gray-100">
              ...
              <div className="mb-6">
                ...
                <textarea
                  value={viceNotes}
                  onChange={(e) => setViceNotes(e.target.value)}
