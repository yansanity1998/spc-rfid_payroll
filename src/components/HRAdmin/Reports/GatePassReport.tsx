...
export const GatePassReport = ({ onBack }: GatePassReportProps) => {
  ...
  const fetchGatePassData = async () => {
    ...
    try {
      ...
      if (requestError) {
        ...
        toast.error('Failed to fetch gate pass requests');
        return;
      }

      if (!requests || requests.length === 0) {
        console.log('[GatePassReport] No gate pass requests found');
        setGatePassData([]);
        return;
      }
  ...
  const getStatistics = () => {
    ...
    const approved = gatePassData.filter(record => record.status === 'Approved').length;
    const pending = gatePassData.filter(record => record.status.includes('Pending')).length;
    const rejected = gatePassData.filter(record => record.status === 'Rejected').length;
          ...
          <div className="mb-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-2xl sm:text-3xl font-bold text-gray-800">Gate Pass Reports</h1>
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="px-6 py-3 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-xl font-semibold shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                >
