...
export const StaffRequest = () => {
  ...
  const fetchExemptedSchedules = async (userId: number) => {
    try {
      const { data, error } = await supabase
        ...
        .select("*")
        .eq("user_id", userId)
        .gte("exemption_date", new Date().toISOString().split("T")[0])
        .order("exemption_date", { ascending: true });

      if (!error) {
        setExemptedSchedules(data || []);
        console.log("[StaffRequest] Exempted schedules:", data);
      }
    } catch (error) {
  ...
  const submitGatePassRequest = async () => {
    try {
      ...
      if (userData) {
        const needsDeanApproval = userData.role === "Staff";
        ...
        if (gatePassForm.attachment) {
          try {
            ...
            if (uploadError) {
              console.error('[StaffRequest] Gate Pass attachment upload error:', uploadError);
              throw uploadError;
            }
            ...
            console.log('[StaffRequest] Gate Pass attachment uploaded successfully:', attachmentUrl);
          } catch (error: any) {
            console.error('[StaffRequest] Error uploading gate pass attachment:', error);
            await Swal.fire({
              title: 'Upload Error!',
        ...
        console.log("[StaffRequest] Submitting Gate Pass request:", requestData);
        ...
        if (!error) {
          ...
          console.error("[StaffRequest] Error submitting request:", error);
          await Swal.fire({
            title: "Error!",
      ...
      if (userData) {
        ...
        if (loanForm.attachment) {
          try {
            ...
            if (uploadError) {
              console.error('[StaffRequest] Loan attachment upload error:', uploadError);
              throw uploadError;
            }
            ...
            attachmentUrl = publicUrl;
            console.log('[StaffRequest] Loan attachment uploaded successfully:', attachmentUrl);
          } catch (error: any) {
            console.error('[StaffRequest] Error uploading loan attachment:', error);
            await Swal.fire({
              title: 'Upload Error!',
              text: `Failed to upload attachment: ${error.message}`,
              icon: 'error',
        ...
        console.log("[StaffRequest] Submitting Loan request:", requestData);
        ...
        if (!error) {
          ...
          console.error("[StaffRequest] Error submitting loan request:", error);
          await Swal.fire({
            title: "Error!",
            text: `Failed to submit request: ${error.message}`,
            icon: "error",
      ...
      if (userData) {
        ...
        if (leaveForm.attachment) {
          try {
            ...
            if (uploadError) {
              console.error('[StaffRequest] Attachment upload error:', uploadError);
              throw uploadError;
            }
            ...
            attachmentUrl = publicUrl;
            console.log('[StaffRequest] Attachment uploaded successfully:', attachmentUrl);
          } catch (error: any) {
            console.error('[StaffRequest] Error uploading attachment:', error);
            await Swal.fire({
              title: 'Upload Error!',
              text: `Failed to upload attachment: ${error.message}`,
              icon: 'error',
        ...
        console.log("[StaffRequest] Submitting Leave request:", requestData);
        ...
        if (!error) {
          ...
          console.error("[StaffRequest] Error submitting leave request:", error);
          await Swal.fire({
            title: "Error!",
            text: `Failed to submit request: ${error.message}`,
            icon: "error",
  ...
  const deleteRequest = async (requestId: number, requestType: string) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        if (error) {
          console.error("[StaffRequest] Error deleting request:", error);
          await Swal.fire({
            title: "Error!",
                  ...
                  <div className="w-14 h-14 bg-white/20 backdrop-blur-md rounded-xl flex items-center justify-center mx-auto mb-3">
                    ...
                    >
                      <path
                        ...
                        strokeLinejoin="round"
                        strokeWidth={2}
                        d="M8 7V3a2 2 0 012-2h4a2 2 0 012 2v4m-6 0h6m-6 0l-2 2m8-2l2 2m-2-2v12a2 2 0 01-2 2H10a2 2 0 01-2-2V9m8 0V9a2 2 0 00-2-2H10a2 2 0 00-2 2v0"
                      />
                    </svg>
                  </div>
                ...
                <div className="p-6 space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setGatePassForm({ ...gatePassForm, time_out: e.target.value })
                        }
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setGatePassForm({ ...gatePassForm, time_in: e.target.value })
                        }
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm"
                      />
                    </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setGatePassForm({ ...gatePassForm, destination: e.target.value })
                      }
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm"
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        setGatePassForm({ ...gatePassForm, purpose: e.target.value })
                      }
                      rows={3}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm"
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      ...
                      {gatePassForm.attachment && (
                        <div className="flex items-center gap-2 p-2 bg-blue-50 rounded-lg border border-blue-200">
                          ...
                          <span className="text-sm text-blue-800 font-medium flex-1">
                            {gatePassForm.attachment.name}
                          </span>
                          <button
                            onClick={() =>
                              setGatePassForm({ ...gatePassForm, attachment: null })
                            }
                      ...
                      <p className="text-xs text-gray-500">
                        Upload supporting document (PDF, DOC, DOCX, JPG, PNG - Max 10MB)
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-6 space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setLoanForm({ ...loanForm, date_needed: e.target.value })
                      }
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 text-sm"
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleLoanFormChange("period_deduction", e.target.value)
                        }
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1.5">
                        Total Periods (15 days each)
                      </label>
                      <input
                        type="number"
                        value={loanForm.total_periods}
                        onChange={(e) =>
                          handleLoanFormChange("total_periods", e.target.value)
                        }
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 text-sm"
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        setLoanForm({ ...loanForm, repayment_terms: e.target.value })
                      }
                      rows={2}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 text-sm"
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setLeaveForm({
                          ...leaveForm,
                          substitute_teacher: e.target.value,
                        })
                      }
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 text-sm"
                ...
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    ...
                    <p className="text-gray-600 text-sm">
                      Your approved requests have exempted these schedules
                    </p>
                  </div>
                </div>
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full border-collapse text-sm">
                  <thead className="bg-red-900 text-white sticky top-0 z-10">
                    <tr>
                      <th className="px-3 sm:px-4 py-3 text-left border-b font-medium">
                        Type
                      </th>
                      <th className="px-3 sm:px-4 py-3 text-left border-b font-medium">
                        Details
                      ...
                      <th className="px-3 sm:px-4 py-3 text-center border-b font-medium">
                        Actions
                      </th>
                    </tr>
                  </thead>
                        ...
                        >
                          ...
                          <td className="px-3 sm:px-4 py-4 border-b border-gray-200 text-center">
                            <div className="flex items-center justify-center gap-2">
                              ...
                              <button
                                onClick={() =>
                                  deleteRequest(request.id, request.request_type)
                                }
          ...
          {showDetailsModal && selectedRequest && (
            <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
              <div className="bg-white rounded-3xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
                <div
                  className={`sticky top-0 z-10 px-8 py-6 border-b border-gray-200 ${
                ...
                <div className="p-8 space-y-6">
                  <div className="flex items-center justify-between pb-6 border-b border-gray-200">
                    <div>
                      ...
                      <span
                        className={`inline-flex items-center px-4 py-2 rounded-xl text-sm font-semibold ${getStatusColor(
                    ...
                    <div className="space-y-6">
                      <div className="bg-green-50 rounded-2xl p-6">
                        ...
                        <div className="space-y-4">
                          ...
                          {(selectedRequest.period_deduction ||
                            selectedRequest.total_periods) && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2 border-t border-green-200">
                              <div className="mt-2">
                                ...
                                <p className="text-lg font-semibold text-gray-800">
                                  ₱
                                  {selectedRequest.period_deduction
                                    ? selectedRequest.period_deduction.toLocaleString(
                                        "en-US",
                                        {
                                          minimumFractionDigits: 2,
                                          maximumFractionDigits: 2,
                              ...
                              <div className="mt-2">
                                ...
                                <p className="text-lg font-semibold text-gray-800">
                                  {selectedRequest.total_periods || "N/A"} periods (15
                                  days each)
                                </p>
                              </div>
                            </div>
                        ...
                        <div className="space-y-4">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                              ...
                              <p className="text-base font-semibold text-gray-800">
                                {selectedRequest.leave_type || "N/A"}
                              </p>
                            </div>
                            <div>
